...
const ConcertFormModal = ({ open, modalId, onCancel, form, onFinish, loading, venues, zoneColors }) => {
  ...
  const getNextPrefixInZone = (zoneIndex) => {
    ...
    targetZone.tiers.forEach(t => {
      if (t && t.rowPrefix && t.rowCount) {
        ...
        let startIndex = 0;
        for (let i = 0; i < prefix.length; i++) startIndex = startIndex * 26 + (prefix.charCodeAt(i) - 64);
    ...
    >
      <Form layout="vertical" form={form} onFinish={onFinish}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '16px' }}>
          ...
          <Form.Item name="concertDate" label="Ngày diễn" dependencies={['saleEndAt']} rules={[{ required: true }, ({ getFieldValue }) => ({ validator(_, value) { if (!value || value.isAfter(getFieldValue('saleEndAt'))) return Promise.resolve(); return Promise.reject(new Error('Ngày diễn phải SAU khi đóng bán vé!')); }, })]}><DatePicker showTime format="DD/MM/YYYY HH:mm" style={{width:'100%'}} disabledDate={(current) => current && current < dayjs().startOf('day')} /></Form.Item>
          <Form.Item name="endDate" label="Kết thúc diễn" dependencies={['concertDate']} rules={[{ required: true }, ({ getFieldValue }) => ({ validator(_, value) { if (!value || value.isAfter(getFieldValue('concertDate'))) return Promise.resolve(); return Promise.reject(new Error('Kết thúc diễn phải SAU khi bắt đầu diễn!')); }, })]}><DatePicker showTime format="DD/MM/YYYY HH:mm" style={{width:'100%'}} disabledDate={(current) => current && current < dayjs().startOf('day')} /></Form.Item>
        </div>
                ...
                <Form.Item noStyle shouldUpdate key={zKey}>
                  {() => {
                    ...
                    return (
                      <Card 
                        size="small" 
                        style={{ marginBottom: 20, borderLeft: `6px solid ${activeColor}`, background: isZoneLocked ? '#fff1f0' : '#fcfcfc', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }} 
                      ...
                      >
                        ...
                        <Form.Item {...restZField} name={[zName, 'layoutConfig', 'x']} hidden><Input /></Form.Item>
  <Form.Item {...restZField} name={[zName, 'layoutConfig', 'y']} hidden><Input /></Form.Item>
  <Form.Item {...restZField} name={[zName, 'layoutConfig', 'w']} hidden><Input /></Form.Item>
  ...
  >
    ...
    />
                                  ...
                                  {!isZoneLocked && (<Button type="dashed" onClick={() => addTier({ price: currentZone.price || 0, rowPrefix: getNextPrefixInZone(zName), rowCount: 1, seatsPerRow: 2 })} block icon={<PlusOutlined />}>Thêm Phân Hạng</Button>)}
                                </>
                              )}
                            </Form.List>
                          )}
              ...
              <Button type="dashed" onClick={() => addZone({ zoneName: `Khu vực ${zoneFields.length + 1}`, price: 10, currency: 'USDT', colorCode: zoneColors[zoneFields.length % zoneColors.length].value, hasSeatMap: true,layoutConfig: { x: 50, y: 150, w: 120, h: 60 }, tiers: [{  price: 10, rowPrefix: 'A', rowCount: 1, seatsPerRow: 2 }] })} block icon={<PlusOutlined />} style={{ height: 40, borderColor: '#1890ff', color: '#1890ff' }}>
                Thêm Khu Vực Mới
              </Button>
            </>
          )}
